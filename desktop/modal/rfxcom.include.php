...
<div class="row row-overflow">
	...
	<div class="col-lg-10" id="div_jeeasyIncludeDisplay">
		...
		<div class="jeeasyDisplay include" style="display:none;">
			...
			<br />
			<center>
				<div class="alert alert-info">{{C'est partie, lançons nous. Cliquez sur le bouton "inclusion" pour lancer l'ajout}}</div>
			</center>
			<br />
			<?php
		...
		<div class="jeeasyDisplay configure" style="display:none;">
			...
			<br />
			<center>
				<div class="alert alert-info">{{Nous aimerions avoir quelques informations pour mieux connaitre votre nouveaux module}}</div>
			</center>
...
<script type="text/javascript">
	...
	$('.bt_jeeasySave').off('click').on('click', function() {
		if (eqLogic_id == null) {
			$('#div_AlertJeeasyInclude').showAlert({
				message: '{{Malheureusement nous ne trouvons pas votre module. Essayez de l\'inclure avant de sauvegarder sa configuration}}',
				level: 'danger'
			});
			return;
		}
	...
	$('body').off('rfxcom::include_mode').on('rfxcom::include_mode', function(_event, _options) {
		if (_options['state'] == 1) {
			if ($('.exclude').attr('data-state') != 0) {
				...
				$('#div_inclusionAlert').showAlert({
					message: '{{Vous etes en mode exclusion. Recliquez sur le bouton d\'exclusion pour sortir de ce mode}}',
					level: 'warning'
				});
			}
		} else {
	...
	$('body').off('rfxcom::includeDevice').on('rfxcom::includeDevice', function(_event, _options) {
		$('#div_inclusionAlert').showAlert({
			message: '{{Félicitation !!! Votre module à bien été inclus. Vous pouvez passer à la suite}}',
			level: 'success'
		});
		eqLogic_id = _options;
		jeedom.eqLogic.byId({
